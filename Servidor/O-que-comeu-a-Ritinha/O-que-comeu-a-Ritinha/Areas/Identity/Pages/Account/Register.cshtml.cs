	...
	public class RegisterModel : PageModel
	{
		...
		public async Task<IActionResult> OnPostAsync(string returnUrl = null)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					// Atribuir o role ao novo utilizador
					...
					if (_userManager.Options.SignIn.RequireConfirmedAccount)
					{
						...
						await _emailSender.SendEmailAsync(Input.Email, "Confirma a tua conta (token)",
							$"Confirma a tua conta usando este token: {token}");

						return RedirectToPage("RegisterConfirmation", new { userId = user.Id });
					}
					else
					{
